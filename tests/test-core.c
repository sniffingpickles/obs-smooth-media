		      ...
		      uint32_t frames, int64_t pts, int64_t arrival)
{
	...
	return audio_buffer_push(buffer, planes, sizes, frames, 48000, 1,
				 1, 1, pts, arrival);
}
...
static void test_buffer_basic(void)
{
	...
	CHECK(push_mono(&buffer, 99, 960, 99 * NS_PER_MS,
			99 * NS_PER_MS));
	memcpy(&first, out->data[0], sizeof(first));
	CHECK(first == 0);
...
static void test_buffer_invalid_input(void)
{
	...
	CHECK(!audio_buffer_push(NULL, planes, sizes, 1, 48000, 1,
				 1, 1, 0, 0));
	CHECK(!audio_buffer_push(&buffer, NULL, sizes, 1, 48000, 1,
				 1, 1, 0, 0));
	CHECK(!audio_buffer_push(&buffer, planes, sizes, 0, 48000, 1,
				 1, 1, 0, 0));
	CHECK(!audio_buffer_push(&buffer, planes, sizes, 1, 0, 1,
				 1, 1, 0, 0));
	CHECK(!audio_buffer_push(&buffer, planes, sizes, 1, 48000, 9,
				 1, 1, 0, 0));

	sizes[0] = 0; /* pointer/size mismatch */
	CHECK(!audio_buffer_push(&buffer, planes, sizes, 1, 48000, 1,
				 1, 1, 0, 0));
	...
	sizes[0] = 1;
	CHECK(audio_buffer_push(&buffer, planes, sizes, 960, 48000, 1,
				1, 1, INT64_MIN, INT64_MIN));
	CHECK(audio_buffer_push(&buffer, planes, sizes, 960, 48000, 1,
				1, 1, INT64_MIN + 1, INT64_MAX));
	struct audio_buffer_stats stats;
	audio_buffer_get_stats(&buffer, &stats);
	CHECK(stats.target_ns == 600 * NS_PER_MS);
	CHECK(stats.max_ns == 850 * NS_PER_MS);
...
static void *clock_writer(void *opaque)
{
	...
	for (int64_t i = 0; i < 10000; i++)
		clock_tracker_record(&stress->clock, i * 2500000,
				     i * 2500100);
